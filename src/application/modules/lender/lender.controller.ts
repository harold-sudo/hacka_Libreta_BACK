...
import { Controller, Inject, ForbiddenException, Get, UseGuards, Req } from '@nestjs/common';
...
export class LenderController {
  ...
  private async profileId(authId: string) {
    ...
    if (!profile || profile.role !== 'LENDER') throw new ForbiddenException('Perfil no autorizado');
    return profile.id;
  }
